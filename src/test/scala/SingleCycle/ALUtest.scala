...
import ALUOP1._
class TestALUSingleCycle ( c : ALU3) extends FreeSpec with ChiselScalatestTester {
"ALU single cycle" in {
test ( new ALU3).withAnnotations( Seq ( VerilatorBackendAnnotation ) ) { c =>
// ALU operations
val array_op = Array ( ALU_ADD , ALU_SUB , ALU_AND , ALU_OR , ALU_XOR , ALU_SLT ,
ALU_SLL , ALU_SLTU , ALU_SRL , ALU_SRA , ALU_COPY_A , ALU_COPY_B , ALU_XXX,BNE,BEQ,BLT,BGE,BLTU,BGEU )
for ( i <- 0 until 100) {
val src_a = Random.nextLong () & 0xFFFFFFFFL//4294967295
...
case BNE=>(src_a!=src_b).asInstanceOf [ Int ]
case BEQ=>(src_a===src_b).asInstanceOf [ Int ]
case BLT=>(src_a < src_b).asInstanceOf [ Int ]
case BGE=>(src_a>=src_b).asInstanceOf [ Int ]
case BLTU=>(src_a.asUInt < src_b.asUInt).asInstanceOf [ Int ]
case BGEU=>(src_a.asUInt>=  src_b.asUInt).asInstanceOf [ Int ]
case _ => 0
}
...
c.io.in_A.poke ( src_a.S)
c.io.in_B.poke ( src_b.S )
