...
class ALU4 extends Module with Config {
    ...
    val sum = io.in_A + Mux(io.alu_Op(0), -io.in_B, io.in_B)//negative first
    ...
    //same width value can be and
    val shiftr = (Cat(io.alu_Op(0) && shin(WLEN-1), shin).asSInt >> shamt)(WLEN-1,0)//shiftr value not comparatable
    val shitfl = Reverse(shiftr)
    val out = 
    Mux(io.alu_Op === ALU_ADD || io.alu_Op === ALU_SUB, sum, 
    Mux(io.alu_Op === ALU_SLT || io.alu_Op === ALU_SLTU, cmp, 
    Mux(io.alu_Op === ALU_SRA || io.alu_Op === ALU_SRL, shitfl,//spelling mistake
    Mux(io.alu_Op === ALU_SLL, shitfl, 
    Mux(io.alu_Op === ALU_AND, (io.in_A & io.in_B),//and 1
